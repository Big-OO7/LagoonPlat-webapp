    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {tab === 'rubric' && (
            <div className="space-y-4">
              {/* Grader-based tasks (new system) */}
              {task.graders && task.graders.length > 0 && (
                <div className="space-y-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Auto-Grading Configuration</h3>
                    <p className="text-sm text-gray-600 mt-1">
                      This task uses {task.graders.length} grader{task.graders.length > 1 ? 's' : ''} for automatic evaluation
                    </p>
                  </div>

                  {task.prompt && (
                    <div className="bg-blue-50 border border-blue-200 rounded p-4">
                      <h4 className="text-sm font-semibold text-blue-900 mb-2">Task Prompt</h4>
                    ...
                    {task.graders.map((grader, index) => (
                      <div key={index} className="border border-gray-300 rounded-lg p-4 bg-white">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <h4 className="font-semibold text-gray-900">{grader.name}</h4>
                            <p className="text-sm text-gray-600 mt-1">
                              Type: <span className="font-mono bg-gray-100 px-2 py-0.5 rounded">{grader.type}</span>
                              {' • '}
                              Weight: <span className="font-medium">{grader.weight}</span>
                            </p>
                          </div>
                        </div>

                        {/* Structure fields for xml/json graders */}
                        {grader.config.structure && grader.config.structure.length > 0 && (
                          <div className="mt-3">
                            <p className="text-sm font-medium text-gray-700 mb-2">Expected Structure:</p>
                            <div className="space-y-2">
                              {grader.config.structure.map((field, fieldIndex) => (
                                <div key={fieldIndex} className="bg-gray-50 border border-gray-200 rounded p-3">
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-gray-600 mt-1">
                                        Comparator: <span className="font-mono">{field.comparator.type}</span>
                                      </p>
                                      {field.comparator.config.expected !== undefined && (
                                        <p className="text-sm text-gray-600">
                                          Expected: <span className="font-mono">{String(field.comparator.config.expected)}</span>
                                        </p>
                                      )}
                                      {field.comparator.type === 'range' && (
                                        <p className="text-sm text-gray-600">
                                          Range: {field.comparator.config.min ?? '-∞'} to {field.comparator.config.max ?? '∞'}
                                        </p>
                                      )}
                                      {field.comparator.type === 'regex' && field.comparator.config.pattern && (
                                        <p className="text-sm text-gray-600">
                                          Pattern: <span className="font-mono">{field.comparator.config.pattern}</span>
                                        </p>
                                      )}
                                      <p className="text-sm text-gray-500 mt-1">
                                        Weight: {field.weight} point{field.weight !== 1 ? 's' : ''}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                        ...
                        {/* Simple graders (text/number) */}
                        {(grader.type === 'text' || grader.type === 'number') && grader.config.expected !== undefined && (
                          <div className="mt-3 bg-gray-50 border border-gray-200 rounded p-3">
                            <p className="text-sm text-gray-600">
                              Expected Answer: <span className="font-mono font-medium">{String(grader.config.expected)}</span>
                            </p>
                          </div>
                        )}
                      </div>
              ...
              {/* Legacy rubric-based tasks */}
              {rubric && (!task.graders || task.graders.length === 0) && (
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="mt-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded">
                      <p className="text-xs text-yellow-800">
                        ⚠️ Legacy rubric-based task. Consider migrating to the new grader system.
                      </p>
                    </div>
                  </div>
              ...
              {/* No graders or rubric */}
              {(!task.graders || task.graders.length === 0) && !rubric && (
                <div className="text-center py-12 bg-gray-50 rounded border-2 border-dashed border-gray-300">
                  <p className="text-gray-600">No graders or rubric configured</p>
                  <p className="text-sm text-gray-500 mt-1">This task cannot be auto-graded</p>
                </div>
              )}
            </div>
          )}
